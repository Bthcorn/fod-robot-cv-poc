...
from confidence_policy import resolve_weights  # noqa: E402  -- same fallback chain, don't duplicate it
...
def main():
    ...
    models = args.models or [args.weights or resolve_weights()]
    ...
    if args.soak:
        print(f"\n=== soak {args.soak}s: {models[0]} {args.formats[0]} {args.precisions[0]} ===")
        return soak(models[0], args.formats[0], args.precisions[0], frames, args.soak, calib)

    rows, sources = [], []
    for weights in models:
        for fmt in args.formats:
            for label in args.precisions:
                print(f"\n=== {Path(weights).name} {fmt} {label} ===")
                row = bench_one(weights, fmt, label, frames, not args.no_val, calib)
                rows.append(row)
