...
Records per cell what AC-3 asks for: FPS, median and p95 latency, the
preprocess/inference/postprocess split, power, and thermals. Published Pi 5
numbers rank the backends in opposite orders and state none of those conditions,
so we measure our own model on our own board. Findings: RESULT.md §4.

Artifacts are built on the Mac (fodcv-export) and read from artifacts/<run-id>/
via exports.json; a missing one falls back to a local export, which cannot work
for litert on aarch64. The `artifact` column is the regression signal -- every
row should read `reused`. An `exported` means the rsync missed and this cell is
...
def apply_threads(n: int):
    ...
    it must be set before those imports -- hence the re-exec. It still misses the
    LiteRT interpreter, so a fair cross-backend sweep also needs
    `taskset -c 0-{n-1}`.
    """
...
def cpu_temp_c() -> float | None:
    ...
    Split out of board_conditions(), which the cooldown must not poll -- it
    shells out to vcgencmd four times per call.
    """
...
def cool_down(target_c: float, timeout_s: float):
    ...
    run()'s drift control *measures* the thermal confound; this removes it. The
    timeout is there so a board that never reaches target still finishes the
    matrix.
    """
    if timeout_s <= 0:
        return
...
def board_power_w():
    ...
    ponytail: an estimate, not a meter -- the PMIC misses the rails it does not
    feed (USB, HATs, NVMe). Constants are the community calibration
    `real_w ~= pmic_sum * 1.15 + 0.6`. NFR-1 wants a USB meter: run one cell
    against it and correct these two numbers.
    """
    try:
        out = subprocess.run(
...
def load_images(paths: list[Path], count: int) -> list:
    ...
    Passing paths to predict() puts a disk read and JPEG decode inside every
    measured call. ndarrays are also what the robot feeds from picamera2.
    """
...
def p95(sorted_ms: list[float]) -> float:
    ...
    reports a latency that wasn't measured.
    """
...
def artifact_for(weights: str, fmt: str, label: str, quantize, calib: Path | None, imgsz: int = IMGSZ):
    ...
    those vary per format and precision, and guessing wrong silently benchmarks
    the wrong file.
    """
    ...
    # Fallback only -- an `exported` in the CSV means the rsync missed.
    if takes_calibration(fmt, quantize) and calib is None:
        # The only calibration set here is the eval split, and calibrating on
        # what we then score against is the leak calib_yaml() exists to close.
        # A FAILED cell is honest; a flattering INT8 number is not.
        raise RuntimeError(
    ...
    # Claim it, exactly as the Mac exporter does -- skipping this leaves the
    # export in Ultralytics' namespace for the next cell to overwrite.
...
def bench_one(weights: str, fmt: str, label: str, frames: list, data_yaml, calib,
    ...
    try:
        ...
        if data_yaml:
            # model.val() builds its own backend, so an accelerator that claims
            # its device exclusively fails the second open
            # (HAILO_OUT_OF_PHYSICAL_DEVICES). Drop ours first -- the backend
            # releases in __del__, and gc.collect() makes that deterministic
            # when a traceback still holds a ref.
            del model
            gc.collect()
        ...
        # A failure is a result too: a cell that cannot build is the answer to
        # whether that runtime can be relied on.
...
def soak(weights: str, fmt: str, label: str, frames: list, seconds: int, calib, imgsz: int = IMGSZ):
    ...
    A 50-run cell never reaches steady state, and AC-3's thermal and power
...
def class_names_in(data_yaml: Path) -> dict[int, str]:
    """The `names:` block. PyYAML, not a hand parser -- ultralytics already
    depends on it, and a bespoke reader only drifts from write_data_yaml."""
...
def eval_split(run: str, dataset: str):
    ...
    On the Pi only `artifacts/<run>/eval/` exists -- data/ is Mac-side. Pinning
    the eval set to the run also stops a v2 benchmark being scored on a v3 split.
    """
...
def check_class_agreement(run: str, data_yaml: Path):
    ...
    It does not error -- a 4-class model on a 1-class set quietly reports
    near-zero mAP, which reads as a bad model, not a mismatched pairing.
    """
...
def run(run_id=CURRENT_RUN, dataset=CURRENT_DATASET, weights=None, models=None, formats=None,
    ...
    # Mac-side only. None rather than the eval yaml -- substituting the split we
    # score against would calibrate INT8 on its own test set.
    ...
    # ondemand idles the A76 at 1.5 GHz and ramps under load, so a cell starting
    # cooled spends part of its 50 runs at 62% clock -- which measured ncnn fp32
    # *slower* after a cooldown (88 -> 100 ms). Unpinned, the cooldown just swaps
    # a thermal confound for a frequency one.
    ...
    for model_weights in models:
        for fmt in formats:
            for label in precisions:
                ...
                # Provenance on the row itself -- a latency without the data
    ...
    # Thermal-drift control: cells run serially on a warming board, so loop
    # position can outweigh runtime. Re-run the first ok cell -- a large delta
    # means the ranking is an artifact, not a result.
